...
def train_fold(fold):
    ...
    n_white=16
    gradient_update_steps=2
    backbone_lr_scale=1


    #dataset
    tensor_path= "/mnt/sda1/Kaggle/PANDA/data/pickled_datas128n64"
    reload_tensor_path="/mnt/sda1/Kaggle/PANDA/data/pickled_datas256n64"
    ...
    calculated_mean=torch.Tensor([0.8021112084388733, 0.7040286064147949, 0.8552106022834778]).to(device).reshape(1,1,3,1,1)
    calculated_std=torch.Tensor([0.18155023455619812, 0.27437373995780945, 0.15957728028297424]).to(device).reshape(1,1,3,1,1)

    #network
    import torchvision.models as models
    from Network import Network
    model = Network(num_classes,'efficientnet-b3',dropout_p=0.5).to(device)
    optimizer = torch.optim.Adam([{'params': model.heads.parameters()},
                                  {'params': model.backbone.parameters(), 'lr': lr*backbone_lr_scale}], lr=lr, weight_decay=weight_decay)
    ...
    for epoch in range(epochs):
        ...
        #freeze_bn(model)
        #print(optimizer)
        #update_lr(optimizer,1e-5,backbone_lr_scale)
        #print(optimizer)
        ...
        #for step in range(1):
            ...
            random_number=np.random.uniform()
            if random_number<0.5:
                x=whiteout(x,n_white)
            x=standardize_batch(x,calculated_mean,calculated_std)
            outputs=model(x)
            del x
        ...
        if (epoch+1)%lr_decay_period==0:
            lr*=lr_decay
            update_lr(optimizer,lr,backbone_lr_scale)
        ...
        validate(model,calculated_mean,calculated_std,csv_file,device,dataset,epoch,batch_size=batch_size*2)
    del model
    del loss
