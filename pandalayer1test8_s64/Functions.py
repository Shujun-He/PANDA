...
def update_lr(optimizer, lr, backbone_lr_scale):
    optimizer.param_groups[0]['lr']=lr
    optimizer.param_groups[1]['lr']=lr*backbone_lr_scale
        #print(param_group)
        #param_group['lr'] = lr
...
def scale_backbone_lr(model,scale=0.1):
    pass
...
def validate(model,mean,std,csv_file,device,dataset,epoch,batch_size=64):
    ...
    uncertainty_penalty=np.asarray(uncertainty_penalty)
    #print(ground_truths.shape)
    #print(predictions.shape)
    score=metrics.cohen_kappa_score(ground_truths,predictions,weights='quadratic')
    K_indices=uncertainty_penalty==1.0
    scoreK=metrics.cohen_kappa_score(ground_truths[K_indices],predictions[K_indices],weights='quadratic')
    scoreR=metrics.cohen_kappa_score(ground_truths[K_indices==False],predictions[K_indices==False],weights='quadratic')
    ...
    with open(csv_file,'a') as f:
        f.write('{},{},{},{},{},{}'.format(epoch+1,acc,score,scoreR,scoreK,losses))
        f.write('\n')
